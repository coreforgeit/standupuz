...
@client_router.callback_query(lambda cb: cb.data.startswith(AdminCB.BACK_START.value))
...
@client_router.callback_query(lambda cb: cb.data.startswith(AdminCB.EDIT_EVENT_LIST.value))
...
@client_router.callback_query(lambda cb: cb.data.startswith(AdminCB.EDIT_HELLO_TEXT_1.value))
...
# команда старт
@client_router.message(StateFilter(AdminStatus.EDIT_HELLO_TEXT))
...
# показывает текст, предлагает поменять
@client_router.callback_query(lambda cb: cb.data.startswith(AdminCB.EDIT_HELLO_TEXT_3.value))
async def edit_hello_text_3(cb: CallbackQuery, state: FSMContext):
    await state.clear()

    await db.Info.update_info(hello_text=cb.message.text, hello_entities=cb.message.entities)
    await cb.message.edit_reply_markup(reply_markup=None)
    await cb.answer('✅ Текст успешно обновлён')


# Обновляет таблицу
@client_router.callback_query(lambda cb: cb.data.startswith(AdminCB.UPDATE_TABLE.value))
